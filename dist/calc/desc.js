...
function buildDescription(description, attacker, defender) {
    ...
    if (description.alliesHealthy) {
        output += Math.min(5, description.alliesHealthy) +
            " ".concat(description.alliesHealthy === 1 ? 'ally' : 'allies', " healthy ");
    }
    if (description.foesFainted) {
        output += Math.min(5, description.foesFainted) +
            " ".concat(description.foesFainted === 1 ? 'foe' : 'foes', " fainted ");
    }
