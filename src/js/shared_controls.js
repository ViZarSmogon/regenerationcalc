...
$(".ability").bind("keyup change", function () {
	...
	var TOGGLE_ABILITIES = ['Flash Fire', 'Intimidate', 'Minus', 'Plus', 'Slow Start', 'Unburden', 'Stakeout', 'Teraform Zero', 'Bull Spirit'];
	...
	if (ability === "Patriarch") {
		$(this).closest(".poke-info").find(".alliesHealthy").show();
	} else {
		$(this).closest(".poke-info").find(".alliesHealthy").val('0');
		$(this).closest(".poke-info").find(".alliesHealthy").hide();

	}
	if (ability === "Wings of Victory") {
...
function createPokemon(pokeInfo) {
	if (typeof pokeInfo === "string") { // in this case, pokeInfo is the id of an individual setOptions value whose moveset's tier matches the selected tier(s)
		...
		return new calc.Pokemon(gen, name, {
			...
			alliesHealthy: parseInt(pokeInfo.find(".alliesHealthy").val()),
